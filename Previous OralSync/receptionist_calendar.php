...
<head>
    ...
    <style>
        ...
        /* Style for disabled dates in picker (standard in most browsers) */
        input[type="date"]:invalid { border: 1px solid red; }
    </style>
</head>
<body>
    ...
    <aside class="sidebar">
        <div class="sidebar-top">
            ...
            <nav class="menu">
                ...
                <a href="receptionist_appointments.php" class="menu-item"><span>📅</span> Appointment</a>
    ...
    <main class="main-content">
        <header class="top-bar">
            <div class="header-right">
                <span class="admin-label">Receptionist | 👤</span>
    ...
    <div class="modal-content">
        ...
        <form action="appointments.php" method="POST" id="appointmentForm">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    <label>Date</label>
                    <input type="date" name="appt_date" id="appt_date" min="<?= $todayDate ?>" value="<?= $todayDate ?>" required>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn-close" id="cancelBtn">Close</button>
                <button type="submit" name="add_appointment" class="btn-save">+ New Appointment</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    // 3. Search & Date Validation
    const apptForm = document.getElementById('appointmentForm');
    const dateInput = document.getElementById('appt_date');

    apptForm.addEventListener('submit', function(e) {
        // Prevent submission if date is in the past (JS Fallback)
        ...
        const val = document.getElementById('patientSearchInput').value;
        const options = document.getElementById('patientList').options;
        let found = false;
        for (let i = 0; i < options.length; i++) {
            if (options[i].value === val) { found = true; break; }
        }
        if (!found) {
            e.preventDefault();
            alert("Error: Please select a registered patient from the list.");
        }
    });
